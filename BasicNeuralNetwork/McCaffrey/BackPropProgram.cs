...
namespace MccaffreyCodingBackProp {
    ...
    public class NeuralNetwork {
        ...
        public double[] ComputeOutputs(double[] xValues) {
            ...
            double[] softOut2 = Softmax(hSums);
            Array.Copy(softOut2, hOutputs, softOut2.Length);
        ...
        private static double[] Softmax(double[] oSums) {
            ...
            */
            ...
            double sum = 0.0;
            for (int i = 0; i < oSums.Length; ++i)
                sum += Math.Exp(oSums[i] - max);

            double[] result = new double[oSums.Length];
            for (int i = 0; i < oSums.Length; ++i) {
                //result[i] = Math.Exp(oSums[i]) / sum;
                double iDunno = max + Math.Log(sum);
                result[i] = Math.Exp(oSums[i] - iDunno);
            }
